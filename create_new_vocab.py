...
def create_new_vocab() -> None:
    with open(vocab_list_file) as in_f, open(new_vocab_list_file, "w") as out_f:
        for index, note in enumerate(in_f, 1):
            ...
            expression_field = furigana_to_kanji(field_collection[0])
            expression_collection = extract_expressions(expression_field)
            index_field = str(index)
            audio_field = "".join(
                map(lambda x: f"[sound:MNN_{x}.mp3]", expression_collection)
            )
            new_note = "\t".join(
                [index_field, expression_field]
                + field_collection
                + [audio_field, tag_field]
            )
